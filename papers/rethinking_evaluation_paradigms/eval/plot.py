...
PLOTS_PATH = PAPER_ROOT / "plots" / "main"
            ...
            # Hardcoded data: (Certified Accuracy, Natural Accuracy, Name)
                ...
                available_methods = [
                    ...
                    if any(
                        ...
                        and result["architecture"] == architecture
                        and abs(float(result["eps"]) - eps) < 1e-6
                        and result["cert_train_method"] == method
                        for result in pareto_results_sorted
                    )
                ]
                ...
                lit_plot = [
                    ...
                    if any(literature_names[method] in result[2] for method in available_methods)
                ]
                ...
                data = [
                    (res['clean_classification_accuracy'], res['certified_accuracy'], f"{res['cert_train_method']} (ours)") for res in pareto_results_sorted if res['dataset'] == dataset and abs(float(res['eps']) - eps) < 1e-6 and res['cert_train_method'] in available_methods and res['total_samples'] > 1 and res['architecture'] == architecture
                ]
                if len(data) == 0:
                    continue
                ...
                methods_tag = "-".join(available_methods)
                for legend in [False, True]:
                    if legend:
                        ...
                        plt.tight_layout()
                        plt.savefig(PLOTS_PATH / f'comparison_{architecture}_{dataset}_{eps}_{methods_tag}.pdf', dpi=300, bbox_inches='tight', transparent=False)
                    else:
                        plt.tight_layout()
                        plt.savefig(PLOTS_PATH / f'comparison_{architecture}_{dataset}_{eps}_{methods_tag}_nolegend.pdf', dpi=300, bbox_inches='tight', transparent=False)
    ...
    for dataset in ['cifar10']:
        for eps in [2/255]:
            ...
            for show_legend in [False, True]:
                ...
                plt.savefig(PLOTS_PATH / f'network_comparison_{method}_{dataset}_{eps}{suffix}.pdf',
...
def plot_testsamples_comparison(all_results):
    ...
    with (PAPER_ROOT / 'results/verification/main/summary_results.json').open() as f:
        results_10k_raw = json.load(f)
    
    # Load sampled results (1k samples)
    with (PAPER_ROOT / 'results/verification/main/summary_results_testsamples1000.json').open() as f:
    ...
    plots_path = PAPER_ROOT / "plots" / "sensitivity" / "test_samples_pairwise"
...
def plot_timeout_comparison(all_results):
    ...
    with (PAPER_ROOT / 'results/verification/main/summary_results_timeout300.json').open() as f:
    ...
    plots_path = PAPER_ROOT / "plots" / "sensitivity" / "timeout_pairwise"
    os.makedirs(plots_path, exist_ok=True)
...
def plot_testsamples_multiple_comparison(all_results, plots_path=None):
    """
    ...
    result_files = {
        10000: PAPER_ROOT / 'results/verification/main/summary_results.json',
        5000: PAPER_ROOT / 'results/verification/main/summary_results_testsamples5000.json',
        2500: PAPER_ROOT / 'results/verification/main/summary_results_testsamples2500.json',
        1000: PAPER_ROOT / 'results/verification/main/summary_results_testsamples1000.json',
    }
    ...
    plots_path = plots_path or (
        PAPER_ROOT / "plots" / "sensitivity" / "test_samples_multiple"
    )
...
def plot_timeout_multiple_comparison(all_results, plots_path=None):
    """
    ...
    result_files = {
        1000: PAPER_ROOT / 'results/verification/main/summary_results.json',
        500: PAPER_ROOT / 'results/verification/main/summary_results_timeout500.json',
        250: PAPER_ROOT / 'results/verification/main/summary_results_timeout250.json',
        100: PAPER_ROOT / 'results/verification/main/summary_results_timeout100.json',
    }
    ...
    plots_path = plots_path or (
        PAPER_ROOT / "plots" / "sensitivity" / "timeout_multiple"
    )
    plots_path.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Pareto-front plots from a combined verification summary."
    )
    parser.add_argument(
        "--summary",
        type=Path,
    ...
    parser.add_argument("--output-dir", type=Path, default=PLOTS_PATH)
    parser.add_argument(
        "--architecture-comparisons",
        action="store_true",
        help="Generate cross-architecture plots (requires an architecture summary).",
    )
    parser.add_argument("--test-sample-comparisons", action="store_true")
    parser.add_argument("--timeout-comparisons", action="store_true")
    parser.add_argument(
        "--comparisons-only",
        action="store_true",
        help="Skip the main method plots when generating budget comparisons.",
    )
    parser.add_argument(
        "--comparison-output-dir",
        type=Path,
        help="Override the standard output directory for comparison plots.",
    )
    args = parser.parse_args()
    PLOTS_PATH = args.output_dir.resolve()
    ...
    if not args.comparisons_only:
        PLOTS_PATH.mkdir(parents=True, exist_ok=True)
        methods = ["mtl_ibp", "sabr", "shi", "crown_ibp", "crown_ibp_nofusion"]
        if args.architecture_comparisons:
            for method in methods:
    ...
    if args.test_sample_comparisons or GENERATE_TESTSAMPLES_COMPARISON:
        ...
        plot_testsamples_multiple_comparison(
            results_sorted, args.comparison_output_dir
        )
    
    if args.timeout_comparisons or GENERATE_TIMEOUT_COMPARISON:
        ...
        plot_timeout_multiple_comparison(results_sorted, args.comparison_output_dir)
