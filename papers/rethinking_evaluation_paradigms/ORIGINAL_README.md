...
Please move the resulting optuna_studies into the folder `./results/hpo/main/optuna_studies` and the resulting network checkpoints into the corresponding folder `./results/hpo/{dataset}_{architecture}_{cert_train_method}{epsilon}`.
...
To evaluate the networks regarding clean accuracy, run `./eval/eval_nat_acc.py` which will populate `./results/verification/clean_accuracy`.
...
There will then exist results files `./results/verification/main/summary_results.json` and `./results/verification/main/summary_results_timeout300_testsamples1000.json`.
...
The resulting plots can be found in `./plots/hpo/importance`


To obtain results plots and tables, please run `./eval/plot.py` and `./tables/main.py`. The generated files can be found in `./plots/main` and `./tables/main` respectively.
...
```
This will generate hypervolume analysis tables in `plots/hpo/front_development`.
...
```
The plot will be saved to `plots/main/motivation_sabr_cifar10.pdf`.

To analyze verification running times and generate tables for different timeouts, run:
```
python ./eval/verification_times.py
```
The results will be stored in `tables/verification_times`.
...
```
This generates a correlation table in `tables/main/verification_correlation.tex`.
